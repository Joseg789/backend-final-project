const validateErrors = (req) => {
  ...
  if (!descripcion || descripcion.trim() === 0)
    errors.push("La descripción es obligatoria.");
  if (!categoria || categoria.trim() === 0)
    errors.push("La categoría es obligatoria.");
  if (!talla || talla.trim() === 0) errors.push("La talla es obligatoria.");
  if (!precio || isNaN(precio) || Number(precio) <= 0)
    errors.push("El precio debe ser un número  y mayor que 0.");
